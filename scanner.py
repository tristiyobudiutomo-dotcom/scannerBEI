...
def flow_score(net_flow_idr):
    """
    net_flow_idr:
    negatif = outflow dari exchange / distribusi
    positif = inflow ke exchange / tekanan jual
    """
    score = 0
    label = "NEUTRAL"
...
def get_flow_proxy(df):
    ...
    - pakai value transaksi hari ini sebagai proksi tekanan uang besar
    ...
    try:
        ...
        if rvol >= 5 and price > float(close.ewm(span=20, adjust=False).mean().iloc[-1]):
            return -traded_value_today
        elif rvol >= 3 and price > float(close.ewm(span=20, adjust=False).mean().iloc[-1]):
            return -traded_value_today * 0.5
        elif rvol >= 4 and price < float(close.ewm(span=20, adjust=False).mean().iloc[-1]):
            return traded_value_today * 0.5
        elif rvol >= 2 and price < float(close.ewm(span=20, adjust=False).mean().iloc[-1]):
            return traded_value_today
...
def level_entry(item, price, support, rvol, atr_pct):
    entry_limit = price * 0.995
    if item["tipe"] == "breakout":
        entry_limit = max(support, price * 0.995)
...
def morning_scan():
    ...
    ihsg_line = regimes[0] if regimes else ""
    ...
    for i, stock in enumerate(stocks):
        ...
        try:
            ...
            breakout_high = float(close.iloc[-1]) > prev_20_high
            ...
            # FILTER KHUSUS: OUTFLOW + BREAKOUT + RVOL
            ...
            if score_m >= 40 and strong_flow_breakout:
                el, sl, tp1, tp2, tp3 = level_entry(item, price, support, rvol, atr_pct)
                item["entry_limit"] = el
                item["stop_loss"] = sl
                item["tp1"] = tp1
                item["tp2"] = tp2
            ...
            if score_r >= 25:
                ...
                el, sl, tp1, tp2, tp3 = level_entry(item_rev, price, support, rvol, atr_pct)
    ...
    msg1 = "🔥 **BREAKOUT MOMENTUM - OUTFLOW + BREAKOUT + RVOL**\n_Entry limit + support intraday_\n\n"
...
# JADWAL WIB - karena GitHub Actions sudah ngatur jam, ini cuma fallback lokal
